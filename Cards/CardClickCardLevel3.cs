...
public class CardClickCardLevel3 : MonoBehaviour
{
	float time_passed = 0;
	...
	public void CardClickCardLevel3Script ()
	{
		GetComponent<Button> ().interactable = false;

		if (BackgroundSequencingInfo.onYellowFieldNow == false) {
			var localCardNumber = int.Parse (GameObject.Find (gameObject.name + "/CardLevel3CardNumber").GetComponent<Text> ().text);

			if (CardDatabase.CardLevel3HasReq [localCardNumber] == true) {
				for (int i = 0; i < 13; i++) { // because there are 13 requirements. If a card should cost something (8-13), then the player still has to have the resources required
					if (CardDatabase.CardLevel3Req1Name [localCardNumber] == i) {
						firstRequirementNumber = i;
						int j = i;
						if (i > 6) {
							j = i - 7;
						}
						if (CardDatabase.CardLevel3Req1Cost [localCardNumber] <= PlrData.plr_resource [PlrData.current_player, j]) { //if the resource of the player higher or equal to the requirement
				...
				if (oneRequirementFulfilled == true) {
					for (int i = 0; i < 13; i++) { 
						if (CardDatabase.CardLevel3Req2Name [localCardNumber] == i) { 
							secondRequirementNumber = i;
							int j = i;
							if (i > 6) {
								j = i - 7; // because I check for 14 things and players got only 7 resources
							}
							if (CardDatabase.CardLevel3Req2Cost [localCardNumber] <= PlrData.plr_resource [PlrData.current_player, j]) { 
	...
	{  
		...
		if (CardDatabase.CardLevel3HasReq [localCardNumber] == true) {
			if (firstRequirementNumber > 6) {
				PlrData.plr_resource [PlrData.current_player, firstRequirementNumber - 7] -= CardDatabase.CardLevel3Req1Cost [localCardNumber];
				gameLogFirstPart = PlrData.plr_name [PlrData.current_player] + " paid " + CardDatabase.CardLevel3Req1Cost [localCardNumber] + " " + PlrData.resource_name_grants [firstRequirementNumber];
				somethingWasPaid = true;
			}
			if (secondRequirementNumber > 6) {
				PlrData.plr_resource [PlrData.current_player, secondRequirementNumber - 7] -= CardDatabase.CardLevel3Req2Cost [localCardNumber];
				gameLogFirstPart += " and " + CardDatabase.CardLevel3Req2Cost [localCardNumber] + " " + PlrData.resource_name_grants [secondRequirementNumber];
			}
	...
	{  
		...
		if (CardDatabase.CardLevel3HasScript [LocalCardNumber] == false) {
			if (CardDatabase.CardLevel3CanAffectAnotherPlayer [LocalCardNumber] == false) {
				float statsOld;
				...
				for (int i = 0; i < 7; i++) { // because there are 7 grants in this card type. In others there are up to 14 as of now.
					if (CardDatabase.CardLevel3GrantsName [LocalCardNumber] == i) { //if you know which grant it is

						whatChanged = PlrData.resource_name_grants [i];
						statsOld = PlrData.plr_resource [PlrData.current_player, i];
						PlrData.plr_resource [PlrData.current_player, i] += CardDatabase.CardLevel3GrantsCost [LocalCardNumber];
						...
						if (CurrentCard.IsPlayedFromHand == true) {
							(PlrData.plr_hand_level3 [PlrData.current_player]).Remove (LocalCardNumber);
						} else {
							CardDatabase.ValidCardsLevel3.Remove (LocalCardNumber);
						}
						PlrData.plr_used_cards_level3 [PlrData.current_player].Add (LocalCardNumber);
				...
				GameLog.AddLog (PlrData.plr_name [PlrData.current_player] + " plays a Level 3 Card called \"" + CardDatabase.CardLevel3Title [LocalCardNumber] + "\". " + gameLogString + " " + whatChanged + ": " + statsOld + " + " + statsDifference + " = " + statsNew + ".");
				giveRewardBool = true;
			...
			GameObject.Find ("Cards").GetComponent<CardScripts> ().PlayScript (3, LocalCardNumber);
			giveRewardBool = true;
	...
	public void AddCardToHand (int LocalCardNumber)
	{
		...
		PlrData.plr_hand_level3 [PlrData.current_player].Add (LocalCardNumber);
	...
	public void DiminishAnotherPlayersResources ()
	{
		...
		whatChanged = PlrData.resource_name_grants [CardDatabase.CardLevel3GrantsName [localCardNumber]];
		statsOld = PlrData.plr_resource [BackgroundSequencingInfo.targetPlayerChosenNumber, CardDatabase.CardLevel3GrantsName [localCardNumber] - 7];
		PlrData.plr_resource [BackgroundSequencingInfo.targetPlayerChosenNumber, CardDatabase.CardLevel3GrantsName [localCardNumber] - 7] -= CardDatabase.CardLevel3GrantsCost [localCardNumber];
		if (CardDatabase.CardLevel3GrantsName [localCardNumber] == 7 || CardDatabase.CardLevel3GrantsName [localCardNumber] == 8) {
			System.Math.Round (PlrData.plr_resource [BackgroundSequencingInfo.targetPlayerChosenNumber, CardDatabase.CardLevel3GrantsName [localCardNumber] - 7], 2);
		}
		statsDifference = statsOld - PlrData.plr_resource [BackgroundSequencingInfo.targetPlayerChosenNumber, CardDatabase.CardLevel3GrantsName [localCardNumber] - 7];
		statsNew = PlrData.plr_resource [BackgroundSequencingInfo.targetPlayerChosenNumber, CardDatabase.CardLevel3GrantsName [localCardNumber] - 7];




		GameObject.Find ("PlayerRewardInfo").GetComponent<Text> ().text = PlrData.plr_name [PlrData.current_player] + " plays a Level 3 Card called \"" + CardDatabase.CardLevel3Title [localCardNumber] + "\" and affects " + PlrData.plr_name [BackgroundSequencingInfo.targetPlayerChosenNumber] + " " + whatChanged + ": " + statsOld + " - " + statsDifference + " = " + statsNew + ".";
		PlrCurrentStatsUpdate.PlrCurrentStatsUpdateClass ();
		GameLog.AddLog (PlrData.plr_name [PlrData.current_player] + " plays a Level 3 Card called \"" + CardDatabase.CardLevel3Title [localCardNumber] + "\" and affects " + PlrData.plr_name [BackgroundSequencingInfo.targetPlayerChosenNumber] + ". " + whatChanged + ": " + statsOld + " - " + statsDifference + " = " + statsNew + ".");
		if (BackgroundSequencingInfo.handIsDisplayed == true) {
			GameObject.Find ("ButtonShowhand").GetComponent<HandUpdate> ().HandUpdateScript ();  //if it WAS displayed previously
		}	
		giveRewardBool = true;
	}
